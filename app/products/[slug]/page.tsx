...
export default async function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  return <div className="mx-auto max-w-screen-2xl px-6 py-12 sm:px-10 lg:px-16 lg:py-16"><div className="grid gap-10 lg:grid-cols-2 lg:gap-16"><ProductGallery productName={product.name} images={product.images} /><div className="max-w-xl lg:pt-4"><p className="text-sm font-semibold uppercase tracking-[0.16em] text-brand-secondary">{product.categories[0]?.name ?? 'Pratima Boutique'}</p><h1 className="mt-3 text-display-md text-brand-text-primary">{product.name}</h1><div className="mt-5 flex items-center gap-3">{isSale && <span className="text-lg text-brand-text-light line-through">£{product.originalPrice?.toFixed(2)}</span>}<span className={`text-2xl font-semibold ${isSale ? 'text-brand-primary' : 'text-brand-text-primary'}`}>£{product.price.toFixed(2)}</span></div>{description && <p className="mt-6 leading-7 text-brand-text-secondary">{description}</p>}<ProductPurchasePanel product={product} /></div></div><RelatedProducts products={related} /></div>;
}
